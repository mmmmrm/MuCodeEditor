...
open class EcmaScriptLanguage(controller: EditorController) : BaseLanguage(controller) {

    override val mLexer: EcmaScriptLexer = EcmaScriptLexer()
    ...
    private fun createAutoCompleteItem(): MutableList<AutoCompleteItem> {
        ...
        mLexer.getKeywordTables().keys.forEach {
            list.add(AutoCompleteItem(it, BaseAutoCompleteHelper.KEYWORD))
        }
        mLexer.getSpecialTables().keys.forEach {
            list.add(AutoCompleteItem(it, BaseAutoCompleteHelper.VARIABLE))
        }
        return list
    }
